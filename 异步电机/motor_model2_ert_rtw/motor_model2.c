...
void motor_model2_step(void)
{
  ...
  for (i = 0; i < 4; i++) {
    rtb_Sum2[i] = ((rtb_Lminrows24col24_0[i + 4] * rtb_Product2_e +
                    rtb_Lminrows24col24_0[i] * rtb_DiscreteTimeIntegrator) +
                   rtb_Lminrows24col24_0[i + 8] * rtb_Gain2_pc) +
      rtb_Lminrows24col24_0[i + 12] * rtb_MultiportSwitch_h_idx_0;
  }
  ...
  if (((rtM->Timing.clockTick1) * 0.0001) >= 0.0001) {
    for (i = 0; i < 16; i++) {
       ...
       *  Switch: '<S16>/Switch1'
       */
      rtb_Product2_e = ((0.0 - W43wr[i]) - rtConstP.Constant4_Value[i]) *
       ...
       *  Gain: '<S27>/wbase*Ts//2 '
       */
      rtb_DiscreteTimeIntegrator = rtConstP.u5_Value_m[i];
      rtb_Sum5[i] = rtb_DiscreteTimeIntegrator + rtb_Product2_e;

      /* Sum: '<S27>/Sum1' incorporates:
       *  Constant: '<S27>/u5'
       *  Gain: '<S27>/wbase*Ts//2 '
       */
      rtb_Lminrows24col24_0[i] = rtb_DiscreteTimeIntegrator - rtb_Product2_e;
    }
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (i_0 = 0; i_0 < 4; i_0++) {
        rtb_Lminrows24col24_0[i_0 + rtb_Sum5_tmp] = ((W43wr[i_0 + 4] *
          rtb_Product2_e + rtb_DiscreteTimeIntegrator * W43wr[i_0]) + W43wr[i_0
          + 8] * rtb_Gain2_pc) + W43wr[i_0 + 12] * rtb_MultiportSwitch_h_idx_0;
      }
    }
    ...
    for (i = 0; i < 4; i++) {
       ...
       */
      rtb_IC[i] = (((W43wr[i + 4] * 0.015707963267948967 * rtb_MultiportSwitch2
                     + 0.015707963267948967 * W43wr[i] * rtb_Switch_idx_0) +
                    W43wr[i + 8] * 0.015707963267948967 * rtb_Subtract2) +
                   W43wr[i + 12] * 0.015707963267948967 * tmp) +
        (((rtb_Lminrows24col24_0[i + 4] * rtb_Product2_e +
           rtb_Lminrows24col24_0[i] * rtb_DiscreteTimeIntegrator) +
          rtb_Lminrows24col24_0[i + 8] * rtb_Gain2_pc) + rtb_Lminrows24col24_0[i
         + 12] * rtb_MultiportSwitch_h_idx_0);
    }
